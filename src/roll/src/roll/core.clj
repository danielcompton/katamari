...
(s/fdef changed
  :args (s/cat :conf any?
               ...
               :build ::build
               :targets (s/? (s/coll-of ::rs/target)))
  :ret (s/map-of ::rs/target #{:unchanged :changed}))

(defn changed
  "Given a previous build state, dry-run a rebuild of selected targets
  and report what subset of targets have changed (and need to be
  rebuilt) since the previous build.

  Will not actually build any products, and so cannot report the deep
  constructive trace fingerprints of the products which would be
  generated by the build.

  Instead reports a map `{<target> #{:changed :unchanged}}` indicating
  whether the target is valid at the previously built fingerprint or
  has been impacted since then.

  WARNING: This function DOES NOT report targets for which no re-build
  will be done. It's possible that some fingerprint doesn't exist in
  the cache and will be re-built. This function MERELY confirms what
  targets in the original build HAVE NOT CHANGED. That is if they are
  re-built due to say cache invalidation, they will be re-built at a
  previously built state.

  Only targets which can be shown to generate new fingerprints will be
  reported as `:changed`."
  [config cache buildgraph build & [targets?]]
  (let [[config buildgraph plan] (plan config buildgraph targets?)]
    (reduce (fn [report target]
              (let [rule   (get-in buildgraph [:targets target])
                    inputs (rule-inputs config buildgraph target rule)]
                (assoc report target
                       (if (some #{:changed}
                                 (map #(get report %)
                                      (apply concat (vals inputs))))
                         ;; easy case - an input is changed
                         :changed
                         ;; hard case - check fingerprints
                         (let [inputs'  (into {}
                                              (map (fn [[key targets]]
                                                     [key (mapv #(get build %) targets)]))
                                              inputs)
                               id (rule-id config buildgraph target rule build inputs')]
                           (if (= id (get-in build [target :id]))
                             :unchanged :changed))))))
            {} (apply concat plan))))
